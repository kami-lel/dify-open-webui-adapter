...
class Test2:  # ================================================================
    ...
    def test_base_url(_, local_cf_app, base_url):
        app = local_cf_app
        assert app.base_url == base_url

    def test_model_id(_, local_cf_app):
        app = local_cf_app
        model_id = "example-chatflow-model"
        assert app.model_id == model_id

    def test_http_header1(_, local_cf_app, chatflow_model1):
        app = local_cf_app
        assert app.http_header(False) == chatflow_model1.http_header(False)

    def test_http_header2(_, local_cf_app, chatflow_model1):
        app = local_cf_app
        assert app.http_header(True) == chatflow_model1.http_header(True)
